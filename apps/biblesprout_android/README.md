...
Word/phrase highlights drawn as **heavy underlines** (no color exists on e-ink — `ReaderView` draws
them under the body layout's line geometry). A pencil in the reader top bar enters **highlight
mode**; long-press stays commentary. Two ways to select a phrase, which can span words, verses,
paragraphs, and pages: the **stylus drag-selects** (press the first word, drag to the last, every
word between underlines in real time on release), or a **finger taps** the first word then the last.
The span previews live; Save persists it, Done exits; a tap/short-press inside an existing highlight
offers Remove. Highlights persist across process death (loaded per chapter on open) and survive
repagination — so they still apply after a font-size change.

- A selection is enumerated against the whole chapter's words in reading order
  (`ReaderActivity.buildChapterWords`/`buildSelection`), so it is **page-independent** — one drag can
  cross pages, and the same word span underlines correctly however the text re-wraps. It persists as
  **one `Highlight` row per verse** the span touches (partial at the ends, whole in the middle).
- **Cross-page drag:** holding the stylus still at the top/bottom edge (a ~0.4s *dwell*, so merely
  *moving* to select near an edge never turns) turns the page mid-drag, keeping the anchor so the
  selection continues onto the next page. The first turn **locks the drag's direction**, so after
  paging forward you can grab the new page's first line without it flipping back.
- `Highlight` entity stores `verse_key` + inclusive `start_word..end_word` (0-based over the verse's
  words); the word span is pagination-independent. `AppIndexDatabase` is at **v3** (`MIGRATION_2_3`
  adds the `highlight` table).
- The mapping between a press / a stored word span and the drawn `StaticLayout` lives in
  `ReaderTypography` (`wordAtOffset`, `highlightRanges` — which merges adjacent words, including
  across verse breaks, into one continuous underline), seeded per page by `ReaderActivity.computeSeeds`
  so a verse split across pages keeps counting its words.
