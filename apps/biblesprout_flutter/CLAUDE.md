# Biblesprout (Flutter — frozen reference)

> **FROZEN.** This is the last single-package Flutter implementation of Biblesprout,
> kept as a self-contained, buildable reference while the app is ported to native
> Android under `apps/biblesprout_android/`. It is not being actively developed.
> Paths below are relative to this directory (`apps/biblesprout_flutter/`). The
> pre-split snapshot is tagged `v0.1-flutter`. Shared scripture/commentary content
> now also lives at the repo root under `data/`; this app keeps its own copy under
> `assets/` so it still builds on its own.
